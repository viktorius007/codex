...
const COMPACTION_CANARY: &str = "PRIVATE-DIAGNOSTIC-COMPACTION-CANARY";
...
async fn legacy_compaction_records_final_payload_and_completed_without_usage() -> Result<()> {
    ...
    mount_sse_once(
        &server,
        sse(vec![
            ev_response_created("before-compaction-response"),
            ev_assistant_message("before-compaction-message", "context to compact"),
            completed_with_usage("before-compaction-response", INITIAL_USAGE),
        ]),
    )
    .await;
    let compact_mock = mount_compact_json_once(
        ...
        json!({
            "output": [{
                "type": "compaction",
                "encrypted_content": "opaque-compact-result",
            }],
        }),
    )
    .await;
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            ...
            config.compact_prompt = Some("compact the test context".to_string());
            let _ = config.features.disable(Feature::RemoteCompactionV2);
        });
    let test = builder.build_with_auto_env(&server).await?;

    test.submit_turn(COMPACTION_CANARY).await?;
    ...
    let request = &archive.records[2];
    let outcome = &archive.records[3];
    ...
    assert_eq!(
        ...
        compact_request.body_bytes().len()
    );
    assert_eq!(
        request["logical"]["body"]["bytes"],
        serde_json::to_vec(&compact_request.body_json())?.len()
    );
    assert_eq!(
        request["logical"]["input"]["count"],
        compact_request.body_json()["input"]
            .as_array()
            .expect("compact request input should be an array")
            .len()
    );
    assert_eq!(request["wire"]["transport"]["endpoint"], "compact");
    assert_eq!(request["wire"]["transport"]["compression"], "none");
    ...
    assert_bytes_exclude(&archive.bytes, &[COMPACTION_CANARY]);

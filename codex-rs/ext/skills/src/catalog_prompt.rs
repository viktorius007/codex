...
const SKILLS_INTRO_WITH_PLUGIN_PACKAGES: &str = "A skill is a set of instructions provided through a `SKILL.md` source. Below is the list of skills that can be used. Each entry includes a name, description, and source locator. `file` locators are on the host filesystem and may use the skill roots table, while `plugin package` locators are stable package identifiers read through `skills.read`.";
...
- Context hygiene:
  ...
  - When variants exist (frameworks, providers, domains), pick only the relevant reference file(s) and note that choice.
- Safety and fallback: If a skill can't be applied cleanly (missing files, unclear instructions), state the issue, pick the next-best approach, and continue."###;
const SKILLS_HOW_TO_USE_WITH_PLUGIN_PACKAGES: &str = r###"- Discovery: The list above is the skills available in this session (name + description + source locator). `file` entries live on the host filesystem; `plugin package` entries are accessed through `skills.read`.
- Trigger rules: If the user names a skill (with `$SkillName` or plain text) OR the task clearly matches a skill's description shown above, you must use that skill for that turn. Multiple mentions mean use them all. Do not carry skills across turns unless re-mentioned.
- Missing/blocked: If a named skill isn't in the list or its source can't be read, say so briefly and continue with the best fallback.
- How to use a skill (progressive disclosure):
  1) After deciding to use a skill, the main agent must read its `SKILL.md` completely before taking task actions. For a `file` entry, expand any listed root alias and open the path. For a `plugin package`, pass the listed locator directly to `skills.read` as `package`. If a read is paginated, follow `next_cursor` until EOF.
  2) When `SKILL.md` references another resource, use the same access mechanism. Resolve filesystem paths relative to the directory containing `SKILL.md`; for plugin skills, pass the complete package-contained resource identifier with the same package to `skills.read`.
  3) If `SKILL.md` points to extra folders such as `references/`, use its routing instructions to identify the files required for the task. The main agent must read each required instruction or reference file itself before acting on it. Do not delegate reading, summarizing, or interpreting skill instructions to a subagent. Subagents may still perform task work when the selected skill allows it.
  4) For filesystem-backed skills, prefer running or patching provided scripts instead of retyping large code blocks. For plugin skills, use `skills.read` and the available tools; do not invent a local path.
  5) Reuse provided assets or templates through the same source access mechanism instead of recreating them.
- Coordination and sequencing:
  - If multiple skills apply, choose the minimal set that covers the request and state the order you'll use them.
  - Announce which skill(s) you're using and why (one short line). If you skip an obvious skill, say why.
...
impl SkillPromptKind {
    ...
    fn intro(self) -> &'static str {
        match self {
            ...
            Self::HostPluginPackages => SKILLS_INTRO_WITH_PLUGIN_PACKAGES,
    ...
    pub(crate) fn usage_instructions(self) -> &'static str {
        match self {
            ...
            Self::HostPluginPackages => SKILLS_HOW_TO_USE_WITH_PLUGIN_PACKAGES,
        }
    }
    ...
    pub(crate) fn alias_instructions(self) -> Option<&'static str> {
        match self {
            ...
            Self::Unaliased | Self::HostAliases | Self::HostPluginPackages => None,
        }
    }
}
...
pub(crate) fn render_available_skills_body(
    ...
    if skill_lines.iter().any(|line| {
        line.contains("(plugin package: ")
            || line.contains("(executor package: ")
            || line.contains("(orchestrator package: ")
    }) {
        lines.push(
